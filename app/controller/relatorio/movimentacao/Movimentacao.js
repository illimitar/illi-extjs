Ext.define('Illi.controller.relatorio.movimentacao.Movimentacao', {
    extend: 'Illi.controller.AbstractController',
//    stores: ['relatorio.Relatorios'],
//    models: ['relatorio.Relatorio'],
    views: [
        'relatorio.movimentacao.Form'
    ],
    refs: [
        {
            ref: 'formRelatorioMovimentacao',
            selector: 'formRelatorioMovimentacao'
        }
    ],
    init: function() {
        var me = this;
        me.control(
                {
                    'formRelatorioMovimentacao button[action=gerar]': {
                        click: me.gerar
                    },
                    'formRelatorioMovimentacao button[action=configurar]': {
                        click: me.configurar
                    },
                    'formRelatorioMovimentacao textfield[name=modelo]': {
    ...
    gerar: function() {
        var form = this.getFormRelatorioMovimentacao().getForm();
        if (form.isValid()) {
            Ext.MessageBox.wait('Gerando Relatório', 'Aguarde...');
            try {
                var modelo = this.getFormRelatorioMovimentacao().down("#modeloRelatorioMovimentacao");
                if (modelo) {
                    ...
                    if (urlModelo) {
                        var produtos = Ext.getStore("relatorioMovimentacaoListaProduto");
                        var id_produtos = [];
