...
Ext.define('Ext.ux.grid.FilterBar', {
    ...
    updateBuffer: 1500, // buffer time to apply filtering when typing/selecting

    columnFilteredCls: Ext.baseCSSPrefix + 'column-filtered', // CSS class to apply to the filtered column header

    renderHidden: false, // renders the filters hidden by default, use in combination with showShowHideButton
    showShowHideButton: true, // add show/hide button in actioncolumn header if found, if not a new small column is created
    showHideButtonTooltipDo: 'Exibir barra de filtros', // button tooltip show
    showHideButtonTooltipUndo: 'Ocultar barra de filtros', // button tooltip hide
    showHideButtonIconCls: 'icon-pesquisar', // button iconCls
    ...
    clearAllButtonIconCls: 'icon-cancelado', // css class with the icon of the clear all button
    clearAllButtonTooltip: 'Limpar todos os filtros', // button tooltip

    forceCreateExtraColumn: false, // force create extra column button
    ...
    autoStoresNullValue: 'ISNULL', // value send to the server to expecify null filter
    ...
    //  filtroInicial: false, //definir filtro inicial sempre,
    stateId: false,
    boolTpl: {
        xtype: 'combo',
        fieldCls: 'filtro',
    ...
    dateTpl: {
        ...
        multiValue: true,
        submitFormat: 'd/m/Y',
        altFormats: "d/m/Y|n/j/Y|n/j/y|m/j/y|n/d/y|m/j/Y|n/d/Y|m-d-y|m-d-Y|d/m|d-m|dm|dmy|dmY|d|d/m/Y|n-j|n/j|d,d",
        submitRangeSeparator: '-',
        operator: 'eq',
        emptyText: "INICIO-FIM"
    },
//    dateTpl: {
//        xtype: 'datefield',
//        editable: true,
...
//    },
    ...
    setup: function(grid) {
        ...
        me.grid = grid;
        me.stateId = (grid.stateId ? 'filtro-' + grid.stateId : false);
        ...
        me.actionColumn = (me.forceCreateExtraColumn === false ? me.grid.down('actioncolumn') || me.grid.down('actioncolumnpro') : false);
        me.extraColumn = null;
        me.clearAllEl = null;
        me.showHideEl = null;
        me.task = Ext.create('Ext.util.DelayedTask');
        me.filterArray = (grid.filtroInicial ? grid.filtroInicial : []);
    ...
    renderFilterBar: function(grid) {
        ...
        me.columns.eachKey(function(key, column) {
            ...
            if (me.enableOperators && column.filter.type != 'bool') { //  && (column.filter.type == 'date' || column.filter.type == 'int' || column.filter.type == 'float' || column.filter.type == 'list')
                var operators = column.filter.operators || ['eq', 'gte', 'lte', 'ne', 'inl'];
                plugins.push({
...
//                        var column = thisField.up('gridcolumn');
//                        console.log("renderFilterBar > listeners > focus", column);
                    },
                    scope: me
                },
                plugins: plugins
    ...
    renderButtons: function() {
        ...
        if (me.showShowHideButton && me.columns.getCount()) {
            ...
            me.showHideEl.on('dblclick', function() {
                var filtro = Illi.app.Local.get(me.stateId);
               // console.log('filtro', filtro);
                if (filtro && filtro !== 'NULL') {
                    var filtros = '';
                    Ext.Object.each(filtro, function(key, value, myself) {
                        filtros += value.property + ' : ' + value.value + '</br>';
                    });
                    Ext.Msg.alert('Filtros Aplicados', filtros);
                } else {
                    Ext.Msg.alert('Filtros Aplicados', 'Nenhum Filtro Aplicado');
    ...
    showInitialFilters: function() {
        ...
        Ext.each(me.filterArray, function(filter) {
            ...
            if (filter.type === 'date') {
                var valor = filter.value;
                field.setRawValue(valor);
                field.fireEvent('select', field, valor);
            } else {
    ...
    applyFilters: function(field) {
        ...
        if (me.filterArray.length > 0) {
            ...
            Illi.app.Local.set(me.stateId, 'NULL');
        ...
        if (me.stateId) {
            //console.log('me.filterArray', me.filterArray);
            Illi.app.Local.set(me.stateId, me.filterArray);
        }
    },
    // private
    ...
    clearFilters: function() {
        ...
        me.fields.eachKey(function(key, field) {
            ...
            field.resumeEvents();
//            if (field.type === 'date') {
//                field.fireEvent('select', field, null);
//            }
